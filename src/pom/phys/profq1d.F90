...
! USES:
    ...
    !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
      ...
      use POM,ONLY: h,a,c,kb,kq,dzz,dz,vh,vhp,wusurf,wvsurf,wubot,wvbot &
           ...
           ,l,sh,sm,kn,kh,gm,gh,zz,q2b,q2,UMOL,GRAV
...
!
     ...
     ! -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
      REAL(RLEN) :: A1,A2,B1,B2,C1,CIWC,COEF1,COEF2,COEF3,COEF4,COEF5,CONST1,DH, &
          E1,E2,E3,KAPPA,P,SMALL,SQ,zco,coef6
      INTEGER :: K,KI
     ! -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
     !  Local Arrays
     ! -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
...
!          TEMP3(K) = (1.-0.40* (P/CC(K)**2))
      ...
      DO K = 2,KB - 1
          Q2B(K) = ABS(Q2B(K))
          Q2LB(K) = ABS(Q2LB(K))
          BOYGR(K) = GRAV* (RHO(K-1)-RHO(K))/ (DZZ(K-1)*DH)! & (G) 
!                     +GRAV**2*2.*1.025/ (CC(K-1)**2+CC(K)**2) (G)
