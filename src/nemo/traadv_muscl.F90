...
CONTAINS
   ...
   SUBROUTINE tra_adv_muscl( kt, kit000, cdtype, p2dt, pun, pvn, pwn,   &
      ...
      !
      CALL wrk_alloc( jpi, jpj, jpk, kjpt, zslpx, zslpy, zwx, zwy )
      !
      IF( kt == kit000 )  THEN
         IF(lwp) WRITE(numout,*)
...
!!gm
      ...
      DO jn = 1, kjpt                                       ! tracer loop
         ...
         zwx(:,:,jpk,jn) = 0.e0   ;   zwy(:,:,jpk,jn) = 0.e0        ! bottom values
         ! interior values
         DO jk = 1, jpkm1
            DO jj = 1, jpjm1      
               DO ji = 1, fs_jpim1   ! vector opt.
                  zwx(ji,jj,jk,jn) = umask(ji,jj,jk) * ( ptb(ji+1,jj,jk,jn) - ptb(ji,jj,jk,jn) )
                  zwy(ji,jj,jk,jn) = vmask(ji,jj,jk) * ( ptb(ji,jj+1,jk,jn) - ptb(ji,jj,jk,jn) )
               END DO
           END DO
         END DO
      ENDDO
      ...
      DO jn = 1, kjpt
         ...
         DO jk = 1, jpkm1                                     ! interior values
            DO jj = 2, jpj
               DO ji = fs_2, jpi   ! vector opt.
                  zslpx(ji,jj,jk,jn) =                    ( zwx(ji,jj,jk,jn) + zwx(ji-1,jj  ,jk,jn) )   &
                     &            * ( 0.25 + SIGN( 0.25, zwx(ji,jj,jk,jn) * zwx(ji-1,jj  ,jk,jn) ) )
                  zslpy(ji,jj,jk,jn) =                    ( zwy(ji,jj,jk,jn) + zwy(ji  ,jj-1,jk,jn) )   &
                     &            * ( 0.25 + SIGN( 0.25, zwy(ji,jj,jk,jn) * zwy(ji  ,jj-1,jk,jn) ) )
               END DO
            END DO
         END DO
         !
         DO jk = 1, jpkm1                                     ! Slopes limitation
            DO jj = 2, jpj
               DO ji = fs_2, jpi   ! vector opt.
                  zslpx(ji,jj,jk,jn) = SIGN( 1., zslpx(ji,jj,jk,jn) ) * MIN(    ABS( zslpx(ji  ,jj,jk,jn) ),   &
                     &                                                 2.*ABS( zwx  (ji-1,jj,jk,jn) ),   &
                     &                                                 2.*ABS( zwx  (ji  ,jj,jk,jn) ) )
                  zslpy(ji,jj,jk,jn) = SIGN( 1., zslpy(ji,jj,jk,jn) ) * MIN(    ABS( zslpy(ji,jj  ,jk,jn) ),   &
                     &                                                 2.*ABS( zwy  (ji,jj-1,jk,jn) ),   &
                     &                                                 2.*ABS( zwy  (ji,jj  ,jk,jn) ) )
               END DO
           END DO
         ...
         DO jk = 1, jpkm1                                     ! interior values
            ...
            DO jj = 2, jpjm1
               DO ji = fs_2, fs_jpim1   ! vector opt.
                  ...
                  zzwx = ptb(ji+1,jj,jk,jn) + xind(ji,jj,jk) * zu * zslpx(ji+1,jj,jk,jn)
                  zzwy = ptb(ji  ,jj,jk,jn) + xind(ji,jj,jk) * zu * zslpx(ji  ,jj,jk,jn)
                  zwx(ji,jj,jk,jn) = pun(ji,jj,jk) * ( zalpha * zzwx + (1.-zalpha) * zzwy )
                  !
                  z0v = SIGN( 0.5, pvn(ji,jj,jk) )
                  zalpha = 0.5 - z0v
                  zv  = z0v - 0.5 * pvn(ji,jj,jk) * zdt / ( e1v(ji,jj) * e2v(ji,jj) * fse3v(ji,jj,jk) )
                  zzwx = ptb(ji,jj+1,jk,jn) + xind(ji,jj,jk) * zv * zslpy(ji,jj+1,jk,jn)
                  zzwy = ptb(ji,jj  ,jk,jn) + xind(ji,jj,jk) * zv * zslpy(ji,jj  ,jk,jn)
                  zwy(ji,jj,jk,jn) = pvn(ji,jj,jk) * ( zalpha * zzwx + (1.-zalpha) * zzwy )
               END DO
            END DO
         END DO
      ENDDO
      ...
      DO jn = 1, kjpt
         DO jk = 1, jpkm1
            DO jj = 2, jpjm1      
               DO ji = fs_2, fs_jpim1   ! vector opt.
                  ...
                  ztra = - zbtr * ( zwx(ji,jj,jk,jn) - zwx(ji-1,jj  ,jk  ,jn)   &
                  &               + zwy(ji,jj,jk,jn) - zwy(ji  ,jj-1,jk  ,jn) )
         ...
         IF( cdtype == 'TRA' .AND. ln_diaptr )  CALL dia_ptr_ohst_components( jn, 'adv', zwy(:,:,:,jn)  )

         ! II. Vertical advective fluxes
         ! -----------------------------
         !                                             !-- first guess of the slopes
         zwx (:,:, 1 ,jn) = 0.e0    ;    zwx (:,:,jpk,jn) = 0.e0    ! surface & bottom boundary conditions
         DO jk = 2, jpkm1                                     ! interior values
            zwx(:,:,jk,jn) = tmask(:,:,jk) * ( ptb(:,:,jk-1,jn) - ptb(:,:,jk,jn) )
         END DO

         !                                             !-- Slopes of tracer
         zslpx(:,:,1,jn) = 0.e0                                  ! surface values
         DO jk = 2, jpkm1                                     ! interior value
            DO jj = 1, jpj
               DO ji = 1, jpi
                  zslpx(ji,jj,jk,jn) =                    ( zwx(ji,jj,jk,jn) + zwx(ji,jj,jk+1,jn) )   &
                     &            * ( 0.25 + SIGN( 0.25, zwx(ji,jj,jk,jn) * zwx(ji,jj,jk+1,jn) ) )
               END DO
            END DO
         END DO
         !                                             !-- Slopes limitation
         ...
         IF( lk_vvl ) THEN    ;   zwx(:,:, 1 ,jn) = 0.e0                      !  variable volume
         ELSE                 ;   zwx(:,:, 1 ,jn) = pwn(:,:,1) * ptb(:,:,1,jn)   ! linear free surface
         ENDIF 
         !
         DO jk = 1, jpkm1                                     ! interior values
            zdt  = p2dt(jk)
            DO jj = 2, jpjm1      
               DO ji = fs_2, fs_jpim1   ! vector opt.
                  ...
                  zzwx = ptb(ji,jj,jk+1,jn) + xind(ji,jj,jk) * zw * zslpx(ji,jj,jk+1,jn)
                  zzwy = ptb(ji,jj,jk  ,jn) + xind(ji,jj,jk) * zw * zslpx(ji,jj,jk  ,jn)
                  zwx(ji,jj,jk+1,jn) = pwn(ji,jj,jk+1) * ( zalpha * zzwx + (1.-zalpha) * zzwy )
               END DO 
            END DO
         END DO
         ...
         DO jk = 1, jpkm1                    ! Compute & add the vertical advective trend
            DO jj = 2, jpjm1      
               DO ji = fs_2, fs_jpim1   ! vector opt.
                  ...
                  ztra = - zbtr * ( zwx(ji,jj,jk,jn) - zwx(ji,jj,jk+1,jn) )
                  ! add it to the general tracer trends
                  pta(ji,jj,jk,jn) =  pta(ji,jj,jk,jn) + ztra
               END DO
            END DO
      ...
      END DO
      !
      CALL wrk_dealloc( jpi, jpj, jpk, kjpt, zslpx, zslpy, zwx, zwy )
      !
