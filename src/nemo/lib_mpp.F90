...
902   IF( ios /= 0 ) CALL ctl_nam ( ios , 'nammpp in configuration namelist', lwp )
   ...
   SUBROUTINE mpp_lnk_4d_t( ptab, kjpt, cd_type, psgn, cd_mpp, pval )
      ...
      !
      REAL(wp), DIMENSION(:,:,:,:,:), ALLOCATABLE ::   zt4ns, zt4sn   ! 3d for north-south & south-north
      ...
      DO jn = 1 , kjpt
         DO jk = 1, jpk
            DO jj = nlcj+1, jpj                 ! added line(s)   (inner only)
               ...
               ptab(1     :nldi-1, jj          ,jk,jn) = ptab(nldi     ,     nlej,jk,jn)
               ptab(nlei+1:nlci  , jj          ,jk,jn) = ptab(     nlei,     nlej,jk,jn)
            END DO
            DO ji = nlci+1, jpi                 ! added column(s) (full)
               ptab(ji           ,nldj  :nlej  ,jk,jn) = ptab(     nlei,nldj:nlej,jk,jn)
               ptab(ji           ,1     :nldj-1,jk,jn) = ptab(     nlei,nldj     ,jk,jn)
               ptab(ji           ,nlej+1:jpj   ,jk,jn) = ptab(     nlei,     nlej,jk,jn)
            END DO
         END DO
        ENDDO
         !
      ...
      CASE ( -1, 0, 1 )                ! all exept 2 (i.e. close case)
         iihom = nlci-nreci
         DO jl = 1, jpreci
            zt4ew(:,jl,:,:,1) = ptab(jpreci+jl,:,:,:)
            zt4we(:,jl,:,:,1) = ptab(iihom +jl,:,:,:)
         END DO
      END SELECT
      !
      !                           ! Migrations
      ...
      IF( nbondj /= 2 ) THEN      ! Read Dirichlet lateral conditions
         ijhom = nlcj-nrecj
         DO jl = 1, jprecj
            zt4sn(:,jl,:,:,1) = ptab(:,ijhom +jl,:,:)
            zt4ns(:,jl,:,:,1) = ptab(:,jprecj+jl,:,:)
         END DO
      ENDIF
      !
      !                           ! Migrations
