MODULE trcrst
   ...
   USE api_bfm,   ONLY: ZEROS, SEAmask
...
CONTAINS
   ...
   SUBROUTINE trc_rst_read( Kbb, Kmm )
      ...
      DO jn = 1, jp_bgc_b
         ...
         D2STATE_BEN(jn,:) = pack(tr_b(:,:,1,jn), SEAmask(:,:,1))
      END DO

#ifdef INCLUDE_PELCO2
      ! ph
      CALL iom_get( numrtr, jpdom_auto, 'ph', ph(:,:,:) )
      D3DIAGNOS(pppH,:) = pack(ph(:,:,:), SEAmask(:,:,:))
#endif
   ...
   SUBROUTINE trc_rst_wri( kt, Kbb, Kmm, Krhs )
      ...
      ! ugly patch for benthic vars
      DO jn = 1, jp_bgc_b
         tr_b(:,:,1,jn) = unpack(D2STATE_BEN(jn,:),SEAmask(:,:,1),ZEROS(:,:,1))
         CALL iom_rstput( kt, nitrst, numrtw, 'TRN'//ctrcnm_b(jn), tr_b(:,:,1,jn) )
      END DO

#ifdef INCLUDE_PELCO2
      ! ph
      ph(:,:,:) = unpack(D3DIAGNOS(pppH,:),SEAmask(:,:,:),ZEROS(:,:,:))
