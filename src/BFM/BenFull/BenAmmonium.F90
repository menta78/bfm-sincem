...
! !USES:
  ...
  use mem
#else
  use mem,  ONLY: K4n, K3n, G2o, D1m, K14n, D2m, K24n, D7m, D2STATE_BEN
  use mem, ONLY: ppK4n, ppK3n, ppG2o, ppD1m, ppK14n, ppD2m, &
    ppK24n, ppD7m, NO_BOXES_XY, &
       BoxNumberXY_ben, InitializeModel, LocalDelta, &
  ...
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  use mem_globalfun,   ONLY: insw,IntegralExp, eTq
...
!
  ...
  do BoxNumberXY_ben=1,NO_BOXES_XY
      ...
      !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
      M4n(BoxNumberXY_ben) = K4n(BoxNumberXY_ben)/ p_poro(BoxNumberXY_ben)/( p_p+ &
        ONE)/( D1m(BoxNumberXY_ben))
      M14n(BoxNumberXY_ben) = K14n(BoxNumberXY_ben)/ p_poro(BoxNumberXY_ben)/( p_p+ &
        ONE)/( D2m(BoxNumberXY_ben)- D1m(BoxNumberXY_ben))
      ...
      !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
        ...
        !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
        zuBT  =   reBTn(BoxNumberXY_ben)/ p_poro(BoxNumberXY_ben)/ D1m(BoxNumberXY_ben)

        !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
        ! Anoxic Mineralization at D1.m, using the exponential distribution
        !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
        zuD1 = max( 1.D-20, reATn(BoxNumberXY_ben))/ p_poro(BoxNumberXY_ben)/ IntegralExp( &
          - alpha, p_d_tot_2- D1m(BoxNumberXY_ben))
      ...
      !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
      diff = p_diff* p_poro(BoxNumberXY_ben)* irrenh(BoxNumberXY_ben)* &
      ...
      !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
      KNH4(BoxNumberXY_ben) = InitializeSet( KNH4(BoxNumberXY_ben), 3, 8)

      call DefineSet( KNH4(BoxNumberXY_ben), LAYERS, LAYER1, &
        LAYER2, D1m(BoxNumberXY_ben), D2m(BoxNumberXY_ben))

      call DefineSet( KNH4(BoxNumberXY_ben), DIFFUSION, FOR_ALL_LAYERS, 0, diff, &
                                                                        dummy)

      call DefineSet( KNH4(BoxNumberXY_ben), POROSITY, FOR_ALL_LAYERS, 0, &
        p_poro(BoxNumberXY_ben), dummy)

      call DefineSet( KNH4(BoxNumberXY_ben), ADSORPTION, FOR_ALL_LAYERS, 0, p_p, &
        dummy)
      ...
      !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
      call DefineSet( KNH4(BoxNumberXY_ben), DOUBLE_DEFINE, 11, EXPONENTIAL_TERM, &
        labda, sK4K3)
      call DefineSet( KNH4(BoxNumberXY_ben), DOUBLE_DEFINE, 12, EXPONENTIAL_TERM, - &
        labda, sK4K3)
      call DefineSet( KNH4(BoxNumberXY_ben), DEFINE, 15, CONSTANT_TERM, dummy, dummy)

      call DefineSet( KNH4(BoxNumberXY_ben), DEFINE, 21, ZERO_EXPONENTIAL_TERM, -alpha, dummy)
      call DefineSet( KNH4(BoxNumberXY_ben), DEFINE, 24, LINEAR_TERM, dummy, dummy)
      call DefineSet( KNH4(BoxNumberXY_ben), DEFINE, 25, CONSTANT_TERM, dummy, dummy)

      call DefineSet( KNH4(BoxNumberXY_ben), DEFINE, 31, ZERO_EXPONENTIAL_TERM, -alpha, dummy)
      call DefineSet( KNH4(BoxNumberXY_ben), DEFINE, 35, CONSTANT_TERM, dummy, dummy)
      ...
      !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
      !4
      call CompleteSet( KNH4(BoxNumberXY_ben), SET_CONTINUITY, FLAG, MASS, dummy)
      !5
      call CompleteSet( KNH4(BoxNumberXY_ben), SET_BOUNDARY, LAYER1, &
        EQUATION, ZERO, value=N4n_Ben(BoxNumberXY_ben))

      select case ( InitializeModel)
        case ( 0 )
          call CompleteSet( KNH4(BoxNumberXY_ben), SET_LAYER_INTEGRAL, LAYER2, &
            LAYER2, dummy, value=K14n(BoxNumberXY_ben))

          call CompleteSet( KNH4(BoxNumberXY_ben), SET_LAYER_INTEGRAL_UNTIL, &
            LAYER3, LAYER3, p_d_tot_2, value=K24n(BoxNumberXY_ben))
 
          call CompleteSet( KNH4(BoxNumberXY_ben), INPUT_TERM, 15, STANDARD, dummy, &
            value=a15)
          ...
          !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
          cK4n = CalculateSet( KNH4(BoxNumberXY_ben), SET_LAYER_INTEGRAL, LAYER1, &
            LAYER1, dummy, ZERO)

          !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
          ! Calculate the adaptation time to the steady-state profile
          !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
          Tau  =   CalculateTau(  sK4K3,  diff,  p_p,  D1m(BoxNumberXY_ben))
          ...
          !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
          cK4n = cK4n+( K4n(BoxNumberXY_ben)- cK4n)* IntegralExp( - LocalDelta/ &
            Tau, ONE)
          ...
          ! constraint.
          !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
          dummy = CalculateSet( KNH4(BoxNumberXY_ben), ADD, 0, 0, dummy, cK4n)
          ...
          !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
          jK4K3n(BoxNumberXY_ben)= sK4K3* CalculateFromSet( KNH4(BoxNumberXY_ben), INTEGRAL, &
            RFLUX, ZERO, D1m(BoxNumberXY_ben))

          call flux(BoxNumberXY_ben, iiBen, ppK4n, ppK3n, jK4K3n(BoxNumberXY_ben) )

          jG2K3o(BoxNumberXY_ben)  =   jK4K3n(BoxNumberXY_ben)* p_qon_nitri
          call flux(BoxNumberXY_ben, iiBen, ppG2o, ppG2o, -( jG2K3o(BoxNumberXY_ben)) )

          !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
          ! Estimation of the Vertical fluxes from the set of transient solutions:
          !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
          jK4N4n = CalculateFromSet( KNH4(BoxNumberXY_ben), &
                                         DERIVATIVE, RFLUX, ZERO, ZERO)
          !avoid too large flxues leading to negative concentrations
          call LimitShift(jK4N4n,N4n_Ben(BoxNumberXY_ben)*Depth_Ben(BoxNumberXY_ben) ,&
                          K4n(BoxNumberXY_ben),p_max_state_change)

          call flux(BoxNumberXY_ben, iiBen, ppK4n, ppK4n, -( jK4N4n) )

          jbotN4n(BoxNumberXY_ben)=jbotN4n(BoxNumberXY_ben)+jK4N4n
        ...
        case ( 1 )    ! Determination of Initial condition:
          ...
          zuD1= reBTn(BoxNumberXY_ben) / p_poro(BoxNumberXY_ben)/   &
            IntegralExp( - alpha, D1m(BoxNumberXY_ben)) *exp(-alpha * D1m(BoxNumberXY_ben))

          !6 
          r  =   exp( - alpha*( D2m(BoxNumberXY_ben)- D1m(BoxNumberXY_ben)))
          call FixProportionCoeff(KNH4(BoxNumberXY_ben),21,31,ONE,r)
          ...
          case(2)
            !8
            call CompleteSet( KNH4(BoxNumberXY_ben), INPUT_TERM, 21, PARAMETER, &
            dummy, value=zuD1)
            !9
!           call CompleteSet( KNH4(BoxNumberXY_ben), SET_LAYER_INTEGRAL, LAYER1, &
!           LAYER1, dummy, value=16.0 * N4n_Ben(BoxNumberXY_ben)* p_poro(BoxNumberXY_ben) * 0.02 )
          end select

          dummy = CalculateSet( KNH4(BoxNumberXY_ben), 0, 0,  0, dummy, dummy)

          jK4K3n(BoxNumberXY_ben) = sK4K3* CalculateFromSet( KNH4(BoxNumberXY_ben), INTEGRAL, &
            RFLUX, ZERO, D1m(BoxNumberXY_ben))
          jG2K3o(BoxNumberXY_ben)  =   jK4K3n(BoxNumberXY_ben)* p_qon_nitri

          if ( jK4K3n(BoxNumberXY_ben) < ZERO) then
              write(LOGUNIT,'(''Negative value calculated for K4n'')') 
              write(LOGUNIT,'(''Minrealisation (reBTT) ='',F10.3)') reBTn(BoxNumberXY_ben)
              write(LOGUNIT,'(''Temperature (ETW) ='',F10.3)') ETW_Ben(BoxNumberXY_ben)
              write(LOGUNIT,'(''Amommonium at s/w/interface (N4n_Ben) ='',F10.3)') N4N_Ben(BoxNumberXY_ben)
              call PrintSet(KNH4(BoxNumberXY_ben),"Error in Initialization of ammonium gradient")
          endif
